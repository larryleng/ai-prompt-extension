...
function classifyError(error) {
  ...
  if (message.includes('404') || message.includes('not found') || message.includes('无记录')) {
    return {
      ...
      category: '数据不存在错误',
      suggestion: '请确认数据库中存在标题为"文生图"的配置记录'
    };
  }
...
async function testSimpleDatabaseCall() {
  ...
  try {
    ...
    const data = await client.getPromptDataByTitle('文生图');
...
async function fetchConfigFromDatabase() {
  ...
  if (!records || records.length === 0) {
    throw new Error('未找到标题为"文生图"的配置记录');
  }
...
function getLocalTemplateConfig() {
  try {
    ...
    // 获取图片模板（因为这是kuozhan.js，处理图片相关）
    const imageTemplate = customTemplateManager.getTemplate(TEMPLATE_TYPES.IMAGE);
    
    if (imageTemplate && imageTemplate.trim()) {
      console.log('✅ 使用本地自定义图片模板');
      
      // 检查是否是JSON格式的配置数据（旧的测试数据）
      try {
        const parsedTemplate = JSON.parse(imageTemplate);
    ...
    return {
      content: '请根据用户输入生成AI绘图提示词',
      template: '请根据用户输入生成AI绘图提示词',
      source: 'local_error'
    };
  }
}
...
/**
 * 最简单的数据库调用 - 直接获取"文生图"记录
 */
async function getPromptConfig(configName = '文生图') {
...
const expansionPrompt = {
  ...
  expandText: async (text, count = CONFIG_CONSTANTS.DEFAULT_COUNT) => {
    ...
    if (!useCloudTemplate) {
      ...
      console.log('🏠 本地模板模式：直接提交用户原始提示词');
      console.log('✅ 生成的用户消息（本地模式）:', {
        role: "user",
        content: text,
    ...
    // 云端模板：使用原有的处理逻辑
    console.log('☁️ 云端模板模式：使用模板处理用户输入');
    ...
    if (record.config) {
      ...
      // 如果config是对象但没有template字段，使用默认提示
        ...
        dbContent = '请根据用户输入生成高质量的AI绘图提示词，包含详细的视觉描述、艺术风格和技术参数。';
      }
    } 
    // 备用字段检查
    else if (record.content) {
